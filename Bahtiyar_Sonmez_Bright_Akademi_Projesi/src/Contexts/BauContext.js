...
const BauContext = createContext();

const BauProvider = ({ children }) => {
    ...
    return <BauContext.Provider value={values}>{children}</BauContext.Provider>;
};

const useBauContext = () => {
    const context = useContext(BauContext);
    if (!context) {
        throw new Error("BauContext kullanılmadan önce BauProvider ile sarmalanmalıdır");
    }
    return context;
};

export { BauProvider, useBauContext };